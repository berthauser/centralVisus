...
public class Articulos {
	...
	@FXML
	private TableColumn<Articulo, Number> colUtilidad;
	@FXML
...
//    	Image image0 = new Image("images/articulos.png");
...
//    	imgLogo.setPreserveRatio(true);
	    ...
	    btnNuevo.setOnAction(e -> {
             ...
             boolean okClicked = showVentana(articulo);
             if (okClicked) {
	    ...
	    btnEliminar.setOnAction(del -> {
	    	if (tblArticulos.getSelectionModel().getSelectedIndex() >= 0) {
	    		...
	    		if (result.isPresent()) {
	    			Future<String> updateResult = ArticuloAccess.updStateBaja(articulo.getCodigobarra());			
	    ...
	    btnModificar.setOnAction(upd -> {
	    	...
	    	if (selectedArticulo!= null) {
	    		showVentana(selectedArticulo);
	    	} else {
	    		Alert exceptionDialog = new Alert(Alert.AlertType.WARNING);
	    		exceptionDialog.setTitle("Sin selecciun");
	    		exceptionDialog.setHeaderText("No hay selecciun");
...
//            lblFechaBaja.textProperty().bind(articulo.fecha_bajaProperty().asString());
	...
	private boolean showVentana(Articulo articulo) {
		try {
			FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/iuArticulos.fxml"));
...
//			scene.getStylesheets().add(getClass().getResource("/styles/style.css").toExternalForm());
			...
			controller.setArticulos(articulo);;
