...
public class ActPreciosMarca {

	@FXML
	private JFXComboBox<Marca> cmbMarcas;
	@FXML
	...
	private ObservableList<Marca> listMarcas = FXCollections.observableArrayList();
	...
	public void initialize() throws FileNotFoundException, IOException, ParseException {
		...
		// Leo las marcas que están en el inventario
		ArticuloAccess.getMarcas(listMarcas);
		cmbMarcas.setItems(listMarcas);
		
		cmbMarcas.getValidators().add(validator);
		cmbMarcas.focusedProperty().addListener((o, oldVal, newVal) -> {
	        if (!newVal) {
	            cmbMarcas.validate();
	        }
	    });

		cmbMarcas.valueProperty().addListener((obs, oldItem, newItem) -> {
			if (!(newItem == null)) {
		...
		Platform.runLater(() -> cmbMarcas.requestFocus());
		...
		btnActualizar.setOnAction(e -> {
			...
			alert.setContentText("¿Desea actualizar la Utilidad de la Marca seleccionada?");
			jm.setScene(alert.getDialogPane().getScene());

			Optional<ButtonType> result = alert.showAndWait();

			if ((result.isPresent()) && (result.get() == ButtonType.OK)) {
				Future<Integer> updateResult = ActPreciosUtilidadAccess.updateUtilidadPorMarca(idMarca, Double.valueOf(txtUtilidad.getText()));
