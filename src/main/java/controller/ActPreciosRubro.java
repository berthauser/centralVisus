...
public class ActPreciosRubro {
	...
	private ObservableList<Rubro> listRubros = FXCollections.observableArrayList();
	...
	public void initialize() throws FileNotFoundException, IOException, ParseException {
		...
		// Leo los rubros que están en el inventario
		ArticuloAccess.getRubros(listRubros);
		cmbRubros.setItems(listRubros);
		...
		cmbRubros.valueProperty().addListener((obs, oldItem, newItem) -> {
			if (!(newItem == null)) {
		...
		Platform.runLater(() -> cmbRubros.requestFocus());
		...
		btnActualizar.setOnAction(e -> {
			...
			alert.setContentText("¿Desea actualizar la Utilidad del Rubro seleccionado?");
			jm.setScene(alert.getDialogPane().getScene());

			Optional<ButtonType> result = alert.showAndWait();

			if ((result.isPresent()) && (result.get() == ButtonType.OK)) {
				Future<Integer> updateResult = ActPreciosUtilidadAccess.updateUtilidadPorRubro(idRubro, Double.valueOf(txtUtilidad.getText()));
