...
public class Lineas {
    @FXML
    private TableView<Linea> tblLineas;
    @FXML
    private TableColumn<Linea, String> colDescripcion;
    @FXML
    private TableColumn<Linea, String> colRubro;
    @FXML
    ...
    private ObservableList<Linea> lineasData = FXCollections.observableArrayList();
...
//        imgHeader.setImage(new Image("/images/ic_assessment_black_36dp.png"));
        imgHeader.setImage(new Image("/resources/images/ic_assessment_black_36dp.png"));
	    ...
	    FilteredList<Linea> filteredData = new FilteredList<>(lineasData, b -> true);
	    ...
	    SortedList<Linea> sortedData = new SortedList<>(filteredData);
	    ...
	    sortedData.comparatorProperty().bind(tblLineas.comparatorProperty());
	    // 5. Add sorted (and filtered) data to the table.
	    tblLineas.setItems(sortedData);
	    
        btnAgregar.setOnAction(ins -> {
        	  Linea linea = new Linea();
              boolean okClicked = showVentana(linea);
              if (okClicked) {
                  lineasData.add(linea);
              }
        });
	    
        btnBorrar.setOnAction(del -> {
        	if (tblLineas.getSelectionModel().getSelectedIndex() >= 0) {
        		Linea linea = tblLineas.getItems().get(tblLineas.getSelectionModel().getSelectedIndex());
                ...
                layout.setBody(new Label("aDesea borrar este item? -> " + linea.getDescripcion()));
                ...
                if (result.isPresent()) {
                	Future<String> deleteResult = LineaAccess.delete(linea);
                	deleteResult.onComplete (ar -> {
                		if (ar.succeeded()) {
                			Platform.runLater(new Runnable() {
                				@Override
                				public void run() {
                					lineasData.remove(filteredData.getSourceIndex(
                							sortedData.getSourceIndex(tblLineas.getSelectionModel().getSelectedIndex())
                							));
                					tblLineas.getSelectionModel().clearSelection();
                				}
                			});
        ...
        btnModificar.setOnAction(upd -> {
        	Linea selectedLinea = tblLineas.getSelectionModel().getSelectedItem();
        	if (selectedLinea!= null) {
        		showVentana(selectedLinea);
            } else {
	...
	private boolean showVentana(Linea linea) {
		try {
//			FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/iuLineas.fxml"));
			FXMLLoader loader = new FXMLLoader(getClass().getResource("/resources/fxml/iuLineas.fxml"));
...
//			scene.getStylesheets().add("/styles/style.css");
			...
			IULineas controller = loader.getController();
	...
	private void inicializarTabla() {
		...
		colRubro.setCellValueFactory(cellData -> cellData.getValue().rubroProperty());
	}

	private void clearTabla() {
		tblLineas.getItems().clear();
	}
